...
async def test_rollup_writes_sentinel_row_with_hourly_cost():
    ...
    assert created["ptu_source_model_id"] == "m1"
...
class _FakeSentinelTable:
    ...
    async def upsert(self, where, data):
        ...
        self.rows[row_key] = {
            ...
            "source": data["create"]["ptu_source_model_id"],
    ...
    async def find_many(self, where=None):
        ...
        return [
            _stored_sentinel_row(team_id, day, model_id, value.get("source"), value.get("model_group"))
...
def _stored_sentinel_row(team_id, day, model_id, source_model_id=None, model_group=None):
    row = MagicMock()
    row.team_id = team_id
    row.date = day
    row.model = model_id
    row.model_group = model_group
    row.ptu_source_model_id = source_model_id
    return row
...
async def test_backfill_falls_back_to_the_name_when_a_row_carries_no_source_model_id():
    """A sentinel row without ptu_source_model_id still counts as priced under its name,
    so the deployment-id check adds rename safety without dropping the original guard."""
    table = _FakeSentinelTable()
    table.seed("t", _day(-1), "m1", 240.0)
    prisma = _prisma_for([_windowed_row(effective_from=_midnight(_day(-1)))], table)
