...
def _smart_router(session_affinity=True, ttl_seconds=777):
...
async def test_claim_pin_uses_redis_attached_after_construction():
    ...
    claimed = await callback._claim_pin(
        cache_key=_session_pin_key("late-redis-session", "key-1"),
        pin_value={"model_id": "our-deployment"},
        ttl_seconds=777,
    )

    assert claimed == {"model_id": "other-pod-winner"}
    assert captured["keys"] == (_session_pin_key("late-redis-session", "key-1"),)
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "model,session_affinity,expect_marker",
    [
        ("smart-router", True, True),
        ("smart-router", False, False),
        ("target-group", True, False),
    ],
    ids=["auto-routed-stamps", "affinity-off-no-stamp", "non-auto-routed-clears"],
)
async def test_pre_routing_hook_stamps_or_clears_the_marker_per_attempt(model, session_affinity, expect_marker):
    """Every routing attempt writes or clears the marker, so a fallback from an
    auto-routed group to a plain group cannot carry a stale marker."""
    router = _smart_router(session_affinity=session_affinity)
    try:
        request_kwargs = {
...
def test_complexity_router_with_session_affinity_registers_affinity_callback():
    enabled = _smart_router()
    disabled = _smart_router(session_affinity=False)
    try:
        assert [
