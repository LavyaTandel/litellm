...
class ComplexityRouter(CustomLogger):
    ...
    @property
    def _uses_session_affinity(self) -> bool:
    ...
    def _with_session_deployment_affinity(
        ...
        if response is None or not self._uses_session_affinity:
            return response
        return response.model_copy(
            update=MappingProxyType({"session_affinity_ttl_seconds": self.config.session_affinity_ttl_seconds})
        )
    ...
    async def async_pre_routing_hook(
        ...
        use_session_affinity: Final = self._uses_session_affinity
